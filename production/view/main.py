...
def external():
    print("iamcontroller")
    time.sleep(5)
    print("iamdoingcomputation")
    return "thisisthepassedvalue"
...
class MainWindow(QMainWindow):
    ...
    def oh_no(self):
        x = external()
        self.l.setText(str(x))
        print("controller set the value")
        # time.sleep(8)
        self.timer.stop()
        self.close()
...
def controller():
    ...
    print("controller loading element")
...
if __name__ == "__main__":
    controller_process = multiprocessing.Process(target=controller)
    view_process = multiprocessing.Process(target=start_gui)
